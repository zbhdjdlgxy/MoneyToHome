    ...
    var imageSource: ImageSource = ImageSource.local(name: ""){
        didSet {
            ...
            case let .network(urlStr):
                ...
                self.imageView.kf.setImage(with: URL(string: encodeString)!, placeholder: UIImage.init(named: PLACEHOLDER_PHOTO), options: nil, progressBlock: nil, completionHandler: nil)
            }
        }
    }
